...
var merge = exports.merge = function merge(obj) {
    var target = void 0;
    var key = void 0;
    var i = 1;
...
var escape = exports.escape = function escape(html, encode) {
    return html.replace(!encode ? /&(?!#?\w+;)/g : /&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;').replace(/'/g, '&#39;');
};

var replace = exports.replace = function replace(regex, opt) {
    regex = regex.source;
    opt = opt || '';
...
inline.normal = (0, _utils.merge)({}, inline);

/**
 * Pedantic Inline Grammar
 */

inline.pedantic = (0, _utils.merge)({}, inline.normal, {
...
inline.gfm = (0, _utils.merge)({}, inline.normal, {
    escape: (0, _utils.replace)(inline.escape)('])', '~|])')(),
    url: /^(https?:\/\/[^\s<]+[^<.,:;"')\]\s])/,
    del: /^~~(?=\S)([\s\S]*?\S)~~/,
    text: (0, _utils.replace)(inline.text)(']|', '~]|')('|', '|https?://|')()
});
...
inline.breaks = (0, _utils.merge)({}, inline.gfm, {
    br: (0, _utils.replace)(inline.br)('{2,}', '*')(),
    text: (0, _utils.replace)(inline.gfm.text)('{2,}', '*')()
});
...
function InlineLexer(links, options) {
    this.options = options || _defaults2.default;
    this.links = links;
    this.rules = inline.normal;
    this.renderer = this.options.renderer || new _renderer2.default();
...
InlineLexer.prototype.output = function staticOutput(src) {
    ...
    while (src) {
        ...
        if (cap = this.rules.tag.exec(src)) {
            ...
            src = src.substring(cap[0].length);
            var addOut = this.options.sanitize && this.options.sanitizer ? this.options.sanitizer(cap[0]) : (0, _utils.escape)(cap[0]);
        ...
        if (cap = this.rules.code.exec(src)) {
            src = src.substring(cap[0].length);
            out += this.renderer.codespan((0, _utils.escape)(cap[2], true));
            continue;
        }
...
InlineLexer.prototype.outputLink = function outputLink(cap, link) {
    var href = (0, _utils.escape)(link.href);
    var title = link.title ? (0, _utils.escape)(link.title) : null;
...
function Lexer(options) {
    ...
    this.tokens.links = {};
    this.options = options || _defaults2.default;
    this.rules = _block2.default.normal;
    if (this.options.gfm) {
        if (this.options.tables) {
            this.rules = _block2.default.tables;
        } else {
            this.rules = _block2.default.gfm;
        }
    }
}
Lexer.rules = _block2.default;
        ...
        if (cap = this.rules.list.exec(src)) {
            ...
            for (; i < l; i++) {
                ...
                if (this.options.smartLists && i !== l - 1) {
                    b = _block2.default.bullet.exec(cap[i + 1])[0];
...
var _inlinelexer = __webpack_require__(3);

var _inlinelexer2 = _interopRequireDefault(_inlinelexer);
...
function Parser(options) {
    ...
    this.token = null;
    this.options = options || _defaults2.default;
    this.options.renderer = this.options.renderer || new _renderer2.default();
...
Parser.prototype.parse = function parse(src) {
    ...
    this.inline = new _inlinelexer2.default(src.links, this.options, this.renderer);
    var tocItems = [];
        ...
        case 'list_start':
            {
                ...
                return this.renderer.list(_body2, ordered);
            }
        ...
        case 'loose_item_start':
            {
                ...
                return this.renderer.listitem(_body4);
            }
        case 'html':
            {
...
block.bullet = /(?:[*+-]|\d+\.)/;
block.item = /^( *)(bull) [^\n]*(?:\n(?!\1bull )[^\n]*)*/;
block.item = (0, _utils.replace)(block.item, 'gm')(/bull/g, block.bullet)();

block.list = (0, _utils.replace)(block.list)(/bull/g, block.bullet)('hr', '\\n+(?=\\1?(?:[-*_] *){3,}(?:\\n+|$))')('def', '\\n+(?=' + block.def.source + ')')();

block.blockquote = (0, _utils.replace)(block.blockquote)('def', block.def)();

block._tag = '(?!(?:' + 'a|em|strong|small|s|cite|q|dfn|abbr|data|time|code' + '|var|samp|kbd|sub|sup|i|b|u|mark|ruby|rt|rp|bdi|bdo' + '|span|br|wbr|ins|del|img)\\b)\\w+(?!:/|[^\\w\\s@]*@)\\b';

block.html = (0, _utils.replace)(block.html)('comment', /<!--[\s\S]*?-->/)('closed', /<(tag)[\s\S]+?<\/\1>/)('closing', /<tag(?:"[^"]*"|'[^']*'|[^'">])*?>/)(/tag/g, block._tag)();

block.paragraph = (0, _utils.replace)(block.paragraph)('hr', block.hr)('heading', block.heading)('lheading', block.lheading)('blockquote', block.blockquote)('tag', '<' + block._tag)('def', block.def)();

/**
 * Normal Block Grammar
 */

block.normal = (0, _utils.merge)({}, block);

/**
 * GFM Block Grammar
 */

block.gfm = (0, _utils.merge)({}, block.normal, {
    fences: /^ *(`{3,}|~{3,})[ .]*(\S+)? *\n([\s\S]*?)\s*\1 *(?:\n+|$)/,
    paragraph: /^/,
    heading: /^ *(#{1,6}) +([^\n]+?) *#* *(?:\n+|$)/
});

block.gfm.paragraph = (0, _utils.replace)(block.paragraph)('(?!', '(?!' + block.gfm.fences.source.replace('\\1', '\\2') + '|' + block.list.source.replace('\\1', '\\3') + '|')();

/**
 * GFM + Tables Block Grammar
 */

block.tables = (0, _utils.merge)({}, block.gfm, {
    nptable: /^ *(\S.*\|.*)\n *([-:]+ *\|[-| :]*)\n((?:.*\|.*(?:\n|$))*)\n*/,
    table: /^ *\|(.+)\n *\|( *[-:]+[-| :]*)\n((?: *\|.*(?:\n|$))*)\n*/
});
...
function marked(src, opt, callback) {
    if (callback || typeof opt === 'function') {
        var _ret = function () {
            ...
            opt = (0, _utils.merge)({}, marked.defaults, opt || {});
            var highlight = opt.highlight;
            var tokens = void 0;
            var pending = void 0;
            var i = 0;
            try {
                tokens = _lexer2.default.lex(src, opt);
            } catch (e) {
                return {
                    v: callback(e)
                };
            ...
            var done = function done(err) {
                ...
                var out = void 0;
                try {
                    out = _parser2.default.parse(tokens, opt);
                } catch (e) {
                    err = e;
                }
                opt.highlight = highlight;
    ...
    try {
        if (opt) opt = (0, _utils.merge)({}, marked.defaults, opt);
        return _parser2.default.parse(_lexer2.default.lex(src, opt), opt);
    } catch (e) {
        e.message += '\nPlease report this to https://github.com/chjj/marked.';
        if ((opt || marked.defaults).silent) {
            return '<p>An error occured:</p><pre>' + (0, _utils.escape)(e.message + '', true) + '</pre>';
        }
        throw e;
    }
}
marked.setOptions = function (opt) {
    (0, _utils.merge)(marked.defaults, opt);
    return marked;
};
...
marked.defaults = _defaults2.default;
marked.Parser = _parser2.default;
marked.parser = _parser2.default.parse;

marked.Renderer = _renderer2.default;

marked.Lexer = _lexer2.default;
marked.lexer = _lexer2.default.lex;

marked.InlineLexer = _inlinelexer2.default;
marked.inlineLexer = _inlinelexer2.default.output;
